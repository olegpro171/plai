...
Apps are registered in `app_main` via `mooncake.installApp(new XxxApp_Packer)`. To **add an app**: create `app_xxx/app_xxx.{h,cpp}` defining both classes, add its header to [apps.h](main/apps/apps.h), and install it in [main/main.cpp](main/main.cpp). Source globs (`apps/*.cpp`, `hal/*.cpp`, etc. in [main/CMakeLists.txt](main/CMakeLists.txt)) use `CONFIGURE_DEPENDS`, so they pick up new files automatically on the next build (no manual `idf.py reconfigure`).
...
- **On-screen keyboard layouts** for text entry are defined in [dialog.cpp](main/apps/utils/ui/dialog.cpp) as the `kbd_layouts[]` array — English (default), Ukrainian (ЙЦУКЕН), and Russian (ЙЦУКЕН). Each `KbdLayout` is two 47-entry arrays (`chars` / `chars_shift`) indexed positionally by the `key_nums[]` physical-key map, a mode label + color, and an `enabled_key` — the NVS key (in the `keyboard` namespace) that toggles the layout, or `nullptr` for an always-on layout (English). `[OPT]` cycles through the **enabled** layouts only: it skips any whose `keyboard`-namespace toggle is off, and since English has no toggle and is always enabled, the cycle is never empty and the skip loop always terminates. The renderer overlays Latin→Cyrillic for any non-English layout. Users turn the non-English layouts on/off in **Settings → Keyboard** (the `keyboard` setting group in [settings.cpp](main/settings/settings.cpp), auto-rendered from metadata). Adding a layout is still mostly additive: append one entry to `kbd_layouts[]` (UTF-8 strings; the `efont_unicode_*` fonts already cover Cyrillic glyphs) with its `enabled_key`, then add a matching `TYPE_BOOL` item to the `keyboard` group and a default line to [nvs.csv](nvs.csv).
- **App launcher icons** are 56×56 **byte-swapped** (big-endian) RGB565 `uint16_t[3136]` arrays with `0x8631` as the transparent color key (the ST7789/LovyanGFX panel convention — a plain little-endian RGB565 converter swaps every color and noises up the edges). Generate one from a PNG with [scripts/png_to_icon.py](scripts/png_to_icon.py); [scripts/icons_to_png.py](scripts/icons_to_png.py) decodes the existing icons back to PNG (see [scripts/README.md](scripts/README.md)).

Apps present: `launcher`, `app_settings`, `app_nodes`, `app_channels`, `app_monitor`, `app_stats`, and `app_charge` (a one-way low-power charging screen — stops the mesh/radio, puts the GPS into PCAS standby, drops the CPU to 80 MHz and dims the backlight; exits only via device restart). `app_graphs` exists but is disabled (`#if 0`).

### Settings & persistence (`main/settings/`)
`Settings` ([settings.h](main/settings/settings.h)) wraps **NVS** with an in-RAM cache and a metadata-driven model (`SettingGroup` → `SettingItem` with type/default/min/max/callback). Values are namespaced (`system`, `keyboard`, `lora`, `security`, `nodeinfo`, `position`, `devmetrics`, …) — see [nvs.csv](nvs.csv) for the full key list and defaults. Supports export/import to SD.
